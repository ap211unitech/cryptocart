...
import { ChevronDown, Files, LoaderCircle } from "lucide-react";
...
import { AllOrder, Order, OrderStatus } from "@/types";
...
const OrderItem = ({ order }: { order: AllOrder }) => {
  return order.orders.map((e, orderId) => {
    return (
      <SingleOrderItem
        key={orderId}
        orderId={orderId}
...
export const SingleOrderItem = ({
  ...
  const { width } = useWindowSize();
  const { mutateAsync: onChangeOrderStatus, isPending: isChangingOrderStatus } =
                  ...
                  return (
                    e.status < s.type && (
                      <DropdownMenuItem
                        ...
                        onClick={() =>
                          handleChangeOrderStatus(buyer, s.type, orderId + 1)
                        }
                      >
                        {s.name}
                      </DropdownMenuItem>
